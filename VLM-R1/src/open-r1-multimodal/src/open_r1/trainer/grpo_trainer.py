    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        for x in inputs:
            ...
            for message in msg:
                ...
                if x['data_type'] in ['single_image', 'multi_view']:
                    for content_item in message.get('content', []):
                        content_type = content_item.get('type')
                        
                        if content_type == 'image' and content_item['image'] is not None:
                            cleaned_content = {
                                'type': 'image',
                                'image': content_item['image']
                            }
                        elif content_type == 'text' and content_item['text'] is not None:
                            cleaned_content = {
                                'type': 'text',
                        ...
                        cleaned_message['content'].append(cleaned_content)
                        
                elif x['data_type'] == 'video':
                    for content_item in message.get('content', []):
                        content_type = content_item.get('type')
                        
                        if content_type == 'text' and content_item['text'] is not None:
                            cleaned_content = {
                                'type': 'text',
                                'text': content_item['text']
                            }
            ...
            all_texts.append(self.processing_class.apply_chat_template(cleaned_msg, tokenize=False, add_generation_prompt=True))
            from qwen_vl_utils import process_vision_info
            images, videos = process_vision_info(cleaned_msg)
            all_images.extend(images if images else [])
            all_videos.extend(videos if videos else [])
            
        all_images = all_images if len(all_images) else None
        all_videos = all_videos if len(all_videos) else None
